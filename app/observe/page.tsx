  ...
  return (
    <div className="min-h-screen py-12 px-6">
      <div className="max-w-xl mx-auto">
        ...
        <p className="text-sm text-stone-500 mb-8">Upload 1–4 photos of your plant or tree.</p>
        ...
        <section className="mb-6">
          {images.length < 4 && (
            <>
              {/* Drag-and-drop / browse (desktop + gallery on mobile) */}
              <div
              ...
              >
                ...
                <p className="text-sm text-stone-600 mb-1">
                  Drop photos here or{' '}
                  <span className="text-forest-700 underline">browse library</span>
                </p>
                <p className="text-xs text-stone-400">
                  Up to {4 - images.length} image{4 - images.length !== 1 ? 's' : ''}
                </p>
              </div>
              ...
              <button
                ...
                className="mt-2 w-full flex items-center justify-center gap-2 border border-stone-200 bg-white hover:bg-stone-50 text-stone-600 text-sm py-2.5 rounded transition-colors"
              >
                <span>📷</span>
            ...
            <div className={`grid grid-cols-4 gap-2 ${images.length < 4 ? 'mt-3' : ''}`}>
              {previews.map((src, i) => (
                <div key={i} className="relative aspect-square">
                  ...
                  <img src={src} alt={`upload ${i + 1}`} className="w-full h-full object-cover rounded" />
                  <button
        ...
        <p className="text-center text-xs text-stone-400 mt-4">
          Photos are analysed by PlantNet and OpenAI. Images may be stored temporarily for quality review.
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center">
      ...
      <p className="text-sm text-stone-500 max-w-xs">
        Your field guide is looking at the images. This takes about 10–20 seconds.
      </p>
    </div>
  )
}
      ...
      <div className="max-w-xl mx-auto">
        ...
        {previews.length > 0 && (
          <div className="flex gap-2 mb-8">
            {previews.map((src, i) => (
              // eslint-disable-next-line @next/next/no-img-element
              <img key={i} src={src} alt={`obs ${i + 1}`} className="h-20 w-20 object-cover rounded" />
            ))}
          </div>
        )}
            ...
            <>
              ...
              {others.length > 0 && (
                <div className="space-y-2">
                  {others.map((c, i) => (
                    <div key={i} className="flex items-center justify-between bg-white/60 border border-stone-100 rounded px-3 py-2">
        ...
        {/* ── Feedback ── */}
        <FeedbackBar requestId={result.request_id} blobUrls={result.blob_urls} />
...
function FeedbackBar({ requestId, blobUrls }: { requestId: string; blobUrls: string[] }) {
  ...
  const submit = async (rating: 'helpful' | 'not_helpful') => {
    ...
    try {
      await fetch('/api/feedback', {
        ...
        body:    JSON.stringify({ request_id: requestId, rating, blob_urls: blobUrls }),
      })
    } catch {
  ...
  return (
    <div className="border border-stone-200 rounded p-4 text-center bg-white">
      {state === 'done' ? (
        <p className="text-sm text-stone-500">✓ Thanks for your feedback</p>
      ) : (
        <>
          <p className="text-xs text-stone-400 uppercase tracking-widest mb-3">Was this helpful?</p>
          <div className="flex gap-3 justify-center">
            <button
              onClick={() => submit('helpful')}
              disabled={state === 'loading'}
              className="flex items-center gap-1.5 px-4 py-2 text-sm border border-stone-200 rounded hover:bg-forest-50 hover:border-forest-300 transition-colors disabled:opacity-40"
            >
              👍 <span className="text-stone-600">Helpful</span>
            </button>
            <button
              onClick={() => submit('not_helpful')}
              disabled={state === 'loading'}
              className="flex items-center gap-1.5 px-4 py-2 text-sm border border-stone-200 rounded hover:bg-red-50 hover:border-red-200 transition-colors disabled:opacity-40"
            >
              👎 <span className="text-stone-600">Not helpful</span>
            </button>
          </div>
        </>
      )}
...
function ConfidenceBadge({ value }: { value: number }) {
  ...
  const color = pct >= 70 ? 'bg-forest-100 text-forest-800'
